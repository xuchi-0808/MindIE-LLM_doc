...
- 使用 MindIE LLM 运行模型推理，推荐从 [快速入门](user_guide/quick_start.md) 开始
...
- 多种量化支持：W8A8、W8A16、W4A8 混合精度、KV Cache INT8 等
- 多维并行策略：张量并行、数据并行、专家并行、上下文并行、序列并行
- Prefill/Decode 分离与混合部署
- SplitFuse 分块调度降低 Prefill 时延
...
- 多模态理解、Function Call、Multi-LoRA 等丰富特性
- 完善的参数配置和环境变量体系
